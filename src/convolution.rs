...
use convolutions_rs::transposed_convolutions::TransposedConvolutionLayer;
        ...
        if let Some(input) = self.input.clone() {
            for i in 0..self.depth {
                for j in 0..self.input_depth {
                    ...
                    let covolver = ConvolutionLayer::new(
                        output_grad_reshape.to_owned(),
                        None,
                        1,
                        Padding::Valid,
                    );
                    let input_single_channel = output_grad.index_axis(Axis(0), j);
